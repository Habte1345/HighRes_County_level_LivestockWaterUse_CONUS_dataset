...
class ANNLivestockWCCAdjuster:
    ...
    def apply_physical_relationship(self, df, y_pred):
        ...
        + RH ↑ => WCC ↓
        + Pr ↑ => WCC ↓
        (All with equal influence in direction only)
        """
        # Normalize to 0–1
        t = (df['temp_county'] - df['temp_county'].min()) / (df['temp_county'].max() - df['temp_county'].min())
        rh = (df['RH_county'] - df['RH_county'].min()) / (df['RH_county'].max() - df['RH_county'].min())
        ...
        # Equal-magnitude directional adjustment
        # Temp increases WCC, RH and Pr decrease it
        adj_factor = (1 + t - rh - pr) / (1 + 1 + 1)  # average directional influence

        # Center around 1 and constrain to stable range
        adj_factor = 1 + 0.3 * (adj_factor - 0.5)
        adj_factor = np.clip(adj_factor, 0.7, 1.3)
